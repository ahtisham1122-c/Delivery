-- Atomic delivery-entry write path.
-- Saves the milk delivery and optional same-screen cash payment in one
-- database transaction. Repeated saves for the same customer/date/rider update
-- the existing delivery row instead of creating duplicate active deliveries.

CREATE UNIQUE INDEX IF NOT EXISTS unique_payment_per_delivery_entry_live
ON dp_payments(linked_delivery_id)
