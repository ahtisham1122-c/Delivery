-- Expanded production integrity checklist after atomic payment/closing work.
...
BEGIN
  ...
  WITH raw_checks AS (
    ...
    UNION ALL
    SELECT 'active_duplicate_payment_client_request_groups', 'critical', count(*)::numeric, '0'
    FROM (
    ...
    UNION ALL
    SELECT 'active_nonadjustment_payments_missing_request_id', 'critical', count(*)::numeric, '0'
    FROM dp_payments
    WHERE coalesce(deleted,false) = false
      AND coalesce(is_adjustment,false) = false
    ...
    UNION ALL
    SELECT 'active_duplicate_closing_groups', 'critical', count(*)::numeric, '0'
    FROM (
      SELECT rider_id, date
    ...
    UNION ALL
    SELECT 'closed_day_unlocked_delivery_rows', 'critical', count(*)::numeric, '0'
    FROM dp_closing_records cr
    JOIN dp_deliveries d ON d.date = cr.date
    LEFT JOIN dp_customers c ON c.id = d.customer_id
    WHERE coalesce(cr.deleted,false) = false
      AND coalesce(d.deleted,false) = false
      AND coalesce(d.is_adjustment,false) = false
      AND (d.rider_id = cr.rider_id OR c.rider_id = cr.rider_id)
      AND coalesce(d.is_locked,false) = false

    UNION ALL
    SELECT 'closing_recomputed_mismatches', 'critical', count(*)::numeric, '0'
    FROM (
      SELECT cr.id
      FROM dp_closing_records cr
      LEFT JOIN LATERAL (
        SELECT round(coalesce(sum(l.liters), 0), 2) AS total
        FROM dp_rider_loads l
        WHERE l.rider_id = cr.rider_id
          AND l.date = cr.date
          AND coalesce(l.deleted,false) = false
      ) loads ON true
      LEFT JOIN LATERAL (
        SELECT round(coalesce(sum(d.liters), 0), 2) AS total
        FROM dp_deliveries d
        LEFT JOIN dp_customers c ON c.id = d.customer_id
        WHERE d.date = cr.date
          AND (d.rider_id = cr.rider_id OR c.rider_id = cr.rider_id)
          AND coalesce(d.deleted,false) = false
          AND coalesce(d.is_adjustment,false) = false
      ) deliveries ON true
      LEFT JOIN LATERAL (
        SELECT round(coalesce(sum(p.amount), 0), 2) AS total
        FROM dp_payments p
        JOIN dp_customers c ON c.id = p.customer_id
        WHERE p.date = cr.date
          AND c.rider_id = cr.rider_id
          AND p.mode = 'Cash'
      ...
      LEFT JOIN LATERAL (
        ...
        FROM dp_expenses e
        WHERE e.rider_id = cr.rider_id
          AND e.date = cr.date
          AND coalesce(e.deleted,false) = false
      ) expenses ON true
      WHERE coalesce(cr.deleted,false) = false
        AND (
          abs(round(coalesce(cr.morning_load_liters,0),2) - loads.total) > 0.01
          OR abs(round(coalesce(cr.app_deliveries_liters,0),2) - deliveries.total) > 0.01
          OR abs(round(coalesce(cr.expected_cash_recovery,0),2) - cash.total) > 0.01
          OR abs(round(coalesce(cr.expense_deductions,0),2) - expenses.total) > 0.01
        )
    ) x

    UNION ALL
    SELECT 'normal_payments_after_rider_close', 'critical', count(*)::numeric, '0'
    FROM dp_closing_records cr
    JOIN dp_customers c ON c.rider_id = cr.rider_id
    JOIN dp_payments p ON p.customer_id = c.id AND p.date = cr.date
    WHERE coalesce(cr.deleted,false) = false
      AND coalesce(p.deleted,false) = false
      AND coalesce(p.is_adjustment,false) = false
      AND p.updated_at > cr.timestamp + interval '5 seconds'
